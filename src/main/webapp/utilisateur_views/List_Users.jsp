...
<body>
	<r:override name="pageTitle">
		<div>
			<h2 class="font-bold col-Russian-violet">Listes des utilisateurs
				de systemes</h2>
			<hr>
		</div>
	</r:override>
	...
	<r:override name="content">
		<div class="container d-flex justify-content-end mx-5 mt-2 mb-3 pe-5"
			id="search">
			<form action="<%=request.getContextPath()%>/Liste_Utilisateurs"
				...
				<button class="btn btn-outline-Dark-Cornflower-bleu round-pill-end"
			...
			<table class="table table-hover">
				...
				<tbody>
					<c:forEach items="${users}" var="u">
						<tr>
							<c:if test="${u.getType_user() ne 'Superviseur' }">
								<td>${u.getNom_user()}${u.getPrenom_user()}</td>
								<td>${u.getTele()}</td>
								<td>${u.getEmail()}</td>
								<td>${u.getLogin()}</td>
								<td>${u.getType_user()}</td>
								<td><img alt="" src="login_views/img/${u.getPhoto()}"
									width="100" height="100"></td>
								<td>${u.getCreated_at()}</td>
								<td>${u.getUpdated_at()}</td>
								<td><div class="d-flex justify-content-around BtnEditing"
										id="BtnEditing">
										<a href="Modifier_Les_informations?id_user=${u.getId_user()}"
		...
		function printDiv() {
			...
			newWin.document
					.write("<html><head>\r\n"
							...
							+ "	}\r\n"
							+ "</style>"
							+ "</head><body onload=\"window.print()\"><h2 class=\"m-5\">Listes des utilisateurs de systemes</h2>"
